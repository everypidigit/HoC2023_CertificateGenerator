...
def generate_certificate(input_image_path, output_image_path, name):
    ...
    elif len(name) >= 26 and len(name) < 30:
        drawCertificate.text((1740, 1420), name, font=certificateFont, fill=(255, 0, 0))

    elif len(name) >= 30:
        drawCertificate.text((1590, 1420), name, font=certificateFont, fill=(255, 0, 0))
...
if __name__ == "__main__":
    DF = pd.read_excel("/Users/daniyarkakimbekov/Workspaces/HoC2023_CertificateGenerator/specialData/Margulan27dec.csv")
    ...
    for i in range(0,len(DF)):
            ...
            certificate_path = "./kazakh/volunteer.jpg"
            
            output_path = "".join(["./certificates/volunteer/", name_for_path, ".jpeg"])
            
            print(f"starting process for user number {i} name {name}")
